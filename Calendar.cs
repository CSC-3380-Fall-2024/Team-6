...
{ 
    ...
    public class Calendar : Control
    {
        ...
        {
            int moveAmount = (!this.IsRightToLeft) ? -1 : 1;
            switch (this.DisplayMode)
            { 
                case CalendarMode.Month:
                { 
                    DateTime? selectedDate = this._blackoutDates.GetNonBlackoutDate(DateTimeHelper.AddDays(this.CurrentDate, moveAmount), moveAmount); 
                    ProcessSelection(shift, selectedDate);
                    break; 
                }
                ...
                case CalendarMode.Year:
                { 
                    DateTime? selectedMonth = DateTimeHelper.AddMonths(this.DisplayDate, moveAmount);
                    OnSelectedMonthChanged(selectedMonth); 
                    break; 
                }
                ...
                case CalendarMode.Decade:
                {
                    DateTime? selectedYear = DateTimeHelper.AddYears(this.DisplayDate, moveAmount);
                    OnSelectedYearChanged(selectedYear); 
                    break;
                } 
            } 
